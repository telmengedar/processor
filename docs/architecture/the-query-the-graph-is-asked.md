...
**Sampling is the configured sampling** — `PROCESSOR_MODEL_TEMPERATURE`, default `0.2`. **This differs from
...
no temperature flag of its own, so the 0.2-versus-0.5 gap is a property of every arm-versus-arm figure it
produces rather than a prediction about one — narrower than the 0-versus-0.5 gap this passage described
before the default moved (#14154), but not closed. §18.3.
...
> **This is the sole statement of the formula, and that is a property to re-derive rather than trust.**
> `git grep -nE '3L|2L|\+ D \+' -- '*.md'` returns every statement of it. **Every hit must be inside
> §7.5** — the two statements above plus this note's own lines, which match their own pattern; the test is the
> *location* of the hits, not their count, because the count moves whenever this note is reworded. **A hit in any
> other section is a second copy.** There was one at `96abe6b` — F-6, still reading `3L + D + 100` after this section had been corrected to `C`,
> so the document stated two values for one constant. The lesson is not that the copy was missed: it is that
> **parameterising one copy relocates the class and does not close it.** Only one site may carry the
> derivation, and the rest cite it.
...
| **F-6** | **`derivationError` lands past the embedding cap for a long input** | §7.5 derives the offset from invariant 1 and states its condition rather than asserting a guarantee | An input over **~2,500** characters — §7.5 derives that threshold and is the only place in this repo that states the formula. **The rounded figure is deliberate and is not to be sharpened here:** the exact value moves whenever a record member is added ahead of `derivationError` — it moved by 9 when `now` arrived (§7.5) — while *roughly 2,500* survives it. A second precise copy is the defect §7.5 records. The field is still on the record and still in the HTTP response; only search reachability is lost — the state #13564 lives with for every field after `candidates`. |
...
**`C == A` is necessary, not sufficient — do not read it as certifying the window** *(added 2026-09-11
round 2, #13705 W-2)*. The bracket is a **one-sided test**: it detects a dirty window, and an
added-then-reversed movement inside the window also produces `C == A`. **For this step that residual is the
conservative direction** — a reversal between B and C yields `C == A` with `B != A`, i.e. a **false
rejection**, costing one re-take. The unsafe direction belongs to the arm-versus-arm uses, not here.

**Why this step is fully bracketed, which the arm-versus-arm uses are not.** A and B are the **same arm on
two binaries**, so they issue the **same query set** — QA verified byte-identical `queries` arrays across
all 25 rows of the pre- and post-change sweeps (#13705). Repeating A therefore probes every neighbourhood B
reads. **#11235 §9.1a's scope limit — the bracket covers only A's queries, 25 of 150 on an arm-versus-arm
comparison — does not reach this step**, and that separation is the reason the criterion this document was
corrected for is now decidable rather than merely better worded.
...
product uses the configured sampling, **default `0.2` since #14154** — this passage described the pre-decision
default of `0` before that decision shipped. `cmd/derive` adds no temperature flag of its own, so that
