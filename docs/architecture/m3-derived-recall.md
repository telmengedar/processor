...
| Package | Change |
|---|---|
| `internal/loop/retrieve.go` **(new)** | `Retrieve(ctx, graph GraphPort, anchor Anchor, queries []string, limit, reserve int) ([]Candidate, error)`. Fan-out, the scoped recall, and fusion. **EXTENDED 2026-09-07:** and two exclusions applied in `fuse` — the anchor's own id (**#11753** §6.2 — the *design*; #12822 is the **ruling about** it and has no §6.2) and every `SelfProduced` row (`fix/exclude-run-records-from-recall`, `docs/architecture/self-produced-exclusion-at-fusion.md`). Both are applied *after* fusion, so a surviving candidate keeps the rank the graph gave it rather than the rank it inherits once rows ahead of it are dropped; **the returned list is therefore at most `limit`, not exactly `limit`.** **Takes no `ModelPort`** — the mechanical boundary of §3 expressed as a signature. The fusion itself is a separate **pure** function with no `ctx` and no port, on `Assemble`'s discipline. **`Retrieve` builds the anchor scope itself**, via `RecallScope(ctx, graph, anchor.ID)`: the caller passes the **anchor** and never a scope, so exactly one copy of scope construction exists in the tree — the caller-builds reading would put a second one in `cmd/eval/sweep.go`, which is **R5 realised and invisible to any single-package test**. `queries[0]` is the raw input and is what the scoped recall carries; the unscoped fan-out covers all of `queries`, `queries[0]` included |
...
| R4 | **More of the graph reachable means more self-produced content reachable** (#11179) | `admit` already cuts self-produced rows (#11158) ~~and the sweep counts them~~ **— corrected 2026-09-07: the sweep can no longer count them. `Retrieve` drops run records before the candidate limit (`docs/architecture/self-produced-exclusion-at-fusion.md`), so they never reach the dispositions `BuildRow` counts and `selfProducedCandidates` reads 0 by construction (#10926 §12 E4).** The risk is closed on the derived/scoped path this row is about and remains open on the supplementary path, which `Retrieve` does not serve. **The smoke rig must report the self-produced count among admitted rows**, which #11179 asks for anyway | ~~a two-turn smoke run admits a `processor-run` record and nothing reports it~~ **NO RUNNABLE FALSIFIER ESTABLISHED, and this was already true before 2026-09-07** — `admit` cuts `SelfProduced` before the byte test (`internal/loop/assemble.go`), so *"admits a `processor-run` record"* names a state the code has never been able to reach. The cell is emptied rather than re-aimed: no mutation whose observed output anyone has quoted would exercise this row |
