...
real content accumulates in every record written from here on.~~ **CORRECTED 2026-09-11
(`docs/architecture/the-aperture-spends-slots-admission-refuses.md`, #13601). The evidence this clause
promised was collected, and it is what ended the arrangement it describes.** Self-produced rows are now
excluded from the candidate aperture **before ranking**, in `fuse` rather than in `admit`, and the
aperture fetches deeper so the freed slots are filled by rows admission can consider. So a run record is
no longer retrieved into an initial assembly's `candidates[]` and no longer carries a cut reason there —
exactly as the anchor has never carried one. **What survives of the clause:** the *supplementary* recall
round does not pass through that aperture, so a record still arrives there, is still cut at admission,
and is still recorded with its similarity and reason. The filtering §6.2 forbids is still not what
happened: the row is not removed from the query, it is refused a candidate slot by the rule admission
was already applying to it. **Self-recall is ruled a capability worth
...
| Recall fails | `502`, **no judgement call**, nothing written | **Assembly failure is run failure.** A model call on an empty context is precisely the confident-answer-from-nothing this project exists to prevent. Falling back to "no context" would be the single worst defect available in this milestone |
...
| Judgement-call cap per run | ~~**3**~~ **6** | ~~One judgement call plus two supplementary rounds. Enough to observe the escalation path; small enough that a loop cannot run away~~ **CORRECTED 2026-09-07 (#13064): raised to 6 on measurement.** The old value bought the escalation path and cost the terminal: across 20 distinct inputs the model calls needed to reach the model's *own* terminal ran **median 4.5, max 9**, so a cap of 3 let **8 of 20** finish, and cut the yardstick task off one call short of its terminal with a file already on disk. **6 is the knee — 17 of 20** — at a median **6.99 s** per extra call. 9 buys the last 3 of 20 and roughly doubles the worst case. **A loop still cannot run away; what would move this number again is a re-measurement of that distribution, not an operator.** **SCOPED `3489a2d`:** this bounds **judgement** calls (`MaxModelCalls`, `turn.go:17`, charged only at `turn.go:284` inside the judge loop). The derivation call at `turn.go:194` is not charged to it, so a run makes up to **seven** model calls in all. The row was titled *"Model call cap per run"* until then; the `CORRECTED 2026-09-07` marker above dates the 3→6 change and not this scoping |
...
| R9 | **Graph latency dominates.** ~2 s per run of graph reads (C20, C23, C29) — measured at M1, when every one of them preceded the model; since `3489a2d` only the anchor read does, and the derivation call with its own 30 s bound sits between it and the rest | Accepted at M1. The two reads are independent and could run concurrently — one obvious optimisation, deliberately not taken until a measurement says it matters |
...
12. **The §6.5 table, every row.** Each is a test. The `graph_unavailable`-before-the-model row is the one
    that matters most: assert that a recall failure produces **no judgement call**. (Since `3489a2d` a
    derivation call precedes retrieval, so *some* model call has already completed; the pinned property is
    that `Judge` is not reached.)
