...
lives in what turn 1 writes to the graph and turn 2 then retrieves from it (DiVoid #11141, #11142).
    ...
    python scripts/smoke.py --input "..." --subject 10422
...
DEFAULT_INPUT = (
    "The variables the binary reads are unset in every scope on this machine. "
    "Does that mean the credential is missing?"
)
DEFAULT_SUBJECT = 10422
...
def stop_server(proc, log_path):
    ...
    try:
        ...
        proc.kill()
        proc.wait(timeout=5)
        print(f"server: did not stop within {SHUTDOWN_GRACE_S}s of the interrupt, killed")
        return
