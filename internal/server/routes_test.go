...
func (s stubGraph) Recall(context.Context, string, int, []int64, loop.UpdateWindow) ([]loop.Candidate, error) {
	if s.recallIdx != nil {
		i := *s.recallIdx
		*s.recallIdx++
...
func (s *stubModel) Judge(_ context.Context, in loop.JudgeInput) (loop.JudgeResult, error) {
	if s.err != nil {
...
func withoutToolRequest(result loop.JudgeResult) loop.JudgeResult {
	if result.Reason != loop.WantsRecall && result.Reason != loop.WantsWrite {
		return result
	}
...
func (g *deadlineGraph) Recall(context.Context, string, int, []int64, loop.UpdateWindow) ([]loop.Candidate, error) {
	return nil, nil
}
...
func (g *blockingGraph) Recall(context.Context, string, int, []int64, loop.UpdateWindow) ([]loop.Candidate, error) {
	return nil, nil
}
