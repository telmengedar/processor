...
const (
	summaryNoCutReason      = "(no reason recorded)"
...
func renderSummaryHeader(b *strings.Builder, record Record, at time.Time) {
	...
	fmt.Fprintf(b, "limits   %d cands / %d B block / %d B suppl / %d calls / %d tok%s\n",
...
func renderSummaryAssembly(b *strings.Builder, record Record) {
	...
	fmt.Fprintf(b, "  admitted (%d, %s of %d B):\n",
		len(admitted), summaryBytes(sumDispositionSizes(admitted)), record.Limits.AssemblyByteBudget)
...
func renderSummaryTools(b *strings.Builder, record Record) {
	...
	for i, call := range record.ToolCalls {
		...
		if call.Tool == ToolRecall {
			fmt.Fprintf(b, "  %d %s%s  %q\n", i+1, call.Tool, source, summaryTrunc(call.Query, summaryRecallRunes))
			renderRecallResults(b, call.Results)
		} else {
			fmt.Fprintf(b, "  %d %s%s  %s  %s\n", i+1, call.Tool, source, call.Path, summaryBytes(call.Bytes))
		}
