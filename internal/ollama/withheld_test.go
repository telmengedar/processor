...
const nativeRecallResponse = `{"message":{"role":"assistant","content":"","tool_calls":[{"function":{"name":"recall","arguments":{"query":"what changed"}}}]},"done":true,"done_reason":"stop"}`
...
func TestACallShapedLikeAToolCallInTheTextOfAToolLessCallIsNeverRecoveredIntoOne(t *testing.T) {
	...
	offered, withheld := judgeBoth(t, responseWithContent(t, capturedUnparsedWrite))

	if offered.Reason != loop.WantsWrite {
		t.Fatalf("with the tools offered the same response gave Reason %q, want %q: the fixture must be one this adapter really does recover a call from, or the withheld arm proves nothing", offered.Reason, loop.WantsWrite)
	}
	...
	if withheld.Answer != capturedUnparsedWrite {
...
func TestANativeToolCallOnAToolLessCallIsRecordedAsAFactAndNeverHonoured(t *testing.T) {
	...
	offered, withheld := judgeBoth(t, nativeRecallResponse)
	...
	if withheld.Reason != loop.Answered {
		t.Fatalf("a call issued with no tool list returned Reason %q, want %q taken from the done reason: an endpoint that answers a tool list it was never sent must not be obeyed", withheld.Reason, loop.Answered)
	}
