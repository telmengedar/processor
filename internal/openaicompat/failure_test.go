...
func preambleFields(t *testing.T, err error) []string {
	t.Helper()
	fields := openAICompatPreamble.FindStringSubmatch(err.Error())
	if fields == nil {
...
func TestOpenAICompatJudgeAttachesTheFailurePreambleToEveryFailureMode(t *testing.T) {
	t.Parallel()

	badJSON, _ := respondingServer(t, http.StatusOK, `{"choices":`)
	rejecting, _ := respondingServer(t, http.StatusInternalServerError, `{"error":{"message":"`+upstreamRejection+`"}}`)
	choiceless, _ := respondingServer(t, http.StatusOK, `{"choices":[]}`)
...
func TestOpenAICompatJudgeCarriesTheUpstreamsOwnSentenceBehindThePreamble(t *testing.T) {
	t.Parallel()

	srv, _ := respondingServer(t, http.StatusInternalServerError, `{"error":{"message":"`+upstreamRejection+`"}}`)
...
func TestOpenAICompatJudgeFailurePreambleNamesTheModelItAskedFor(t *testing.T) {
	...
	c := NewClient("http://host.example", "ai/llama3.2", "", loop.Sampling{}, refusingClient())

	if got := preambleFields(t, judgeFailure(t, c))[1]; got != "ai/llama3.2" {
		t.Fatalf("the preamble names model %q, want the model id the client was built with", got)
	}
}

func TestOpenAICompatJudgeFailurePreambleNamesTheRouteBeneathTheBaseURL(t *testing.T) {
	t.Parallel()
...
func TestOpenAICompatJudgeFailurePreambleRedactsUserinfoFromTheEndpointItNames(t *testing.T) {
	t.Parallel()
...
func TestOpenAICompatJudgeFailurePreambleSizesTheRequestTheEndpointActuallyReceived(t *testing.T) {
	t.Parallel()

	srv, captured := respondingServer(t, http.StatusInternalServerError, `{"error":{"message":"nope"}}`)
	c := NewClient(srv.URL, "model-x", "", loop.Sampling{}, srv.Client())
...
func TestOpenAICompatJudgeFailurePreambleNamesTheBoundTheSuppliedClientActuallyCarries(t *testing.T) {
	t.Parallel()
...
func TestOpenAICompatJudgeFailurePreambleNamesTheDefaultBoundWhenNoClientIsSupplied(t *testing.T) {
	t.Parallel()
...
func TestOpenAICompatJudgeFailurePreambleTimesTheCallAgainstTheBoundItNames(t *testing.T) {
	t.Parallel()
...
func TestOpenAICompatJudgeAddsNoPreambleToACallThatSucceeds(t *testing.T) {
	t.Parallel()

	srv, _ := capturingServer(t, stopResponse)
	c := NewClient(srv.URL, "model-x", "", loop.Sampling{}, srv.Client())

	result, err := c.Judge(context.Background(), judgeInput())
	if err != nil {
