...
func rowDispositions(ctx context.Context, graph loop.GraphPort, row eval.Row, derivations eval.Derivations) ([]string, []loop.Disposition, bool, error) {
	...
	retrieval, err := loop.Retrieve(ctx, graph, anchor, derivations.QueriesFor(row), loop.CandidateLimit, loop.RecallScopeReserve)
	if err != nil {
		return nil, nil, true, err
	}

	_, dispositions := loop.Assemble(anchor, retrieval.Candidates, loop.AssemblyByteBudget)
	return retrieval.Queries, dispositions, true, nil
}
